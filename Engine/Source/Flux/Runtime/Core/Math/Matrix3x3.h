...
namespace Flux {
	...
	struct Matrix3x3
	{
		...
		inline static Matrix3x3 Inverse(const Matrix3x3& m)
		{
			...
			Matrix3x3 result;
			result[0][0] =  ((m[1][1] * m[2][2]) - (m[1][2] * m[2][1])) * invDeterminant;
			result[1][0] = -((m[1][0] * m[2][2]) - (m[1][2] * m[2][0])) * invDeterminant;
			result[2][0] =  ((m[1][0] * m[2][1]) - (m[1][1] * m[2][0])) * invDeterminant;
			result[0][1] = -((m[0][1] * m[2][2]) - (m[0][2] * m[2][1])) * invDeterminant;
			result[1][1] =  ((m[0][0] * m[2][2]) - (m[0][2] * m[2][0])) * invDeterminant;
			result[2][1] = -((m[0][0] * m[2][1]) - (m[0][1] * m[2][0])) * invDeterminant;
			result[0][2] =  ((m[0][1] * m[1][2]) - (m[0][2] * m[1][1])) * invDeterminant;
			result[1][2] = -((m[0][0] * m[1][2]) - (m[0][2] * m[1][0])) * invDeterminant;
			result[2][2] =  ((m[0][0] * m[1][1]) - (m[0][1] * m[1][0])) * invDeterminant;
			return result;
		}
